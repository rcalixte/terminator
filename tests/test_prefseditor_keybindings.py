...
@pytest.mark.parametrize(
    ...
    [
        ...
        # 3) `Ctrl+Shift+Up` shouldn't change
        (
            (Gdk.KEY_Up, CONTROL_SHIFT_MOD, 111),
            (Gdk.KEY_Up, CONTROL_SHIFT_MOD),
        ),
        # 4) `Ctrl+Shift+[` should become `Ctrl+{`
        (
            (Gdk.KEY_bracketleft, CONTROL_SHIFT_MOD, 34),
            (Gdk.KEY_braceleft, Gdk.ModifierType.CONTROL_MASK),
        ),
        # 5) `Shift+Tab` shouldn't change
        (
